    ...
    def get_input_img(self, image, crop=False, crop_size=512):
        ...
        if crop:
            ...
            train_img, _, ratio = utils.resize_and_pad_image(image,
                                                             ...
                                                             crop_size,
                                                             jitter=None)
            train_img = tf.keras.applications.resnet.preprocess_input(
                train_img)
    ...
    def detect_single_image(self, image, crop_sizes=[], tiling=False):
        ...
        if tiling:
            input_img, image, ratio = self.get_input_img(
                image, crop=True, crop_size=self.tiling_size)

            detections = self.inference_model.predict_on_batch(
                tf.concat(input_img, 0))
        ...
        for crop_size in crop_sizes:
            input_img, image, ratio = self.get_input_img(
                image, crop=False, crop_size=crop_size)
...
def run_prediction(args):
    ...
    for file_path in tqdm(image_files):
        ...
        if save_dir:
            ...
            visualize_detections(image, boxes, cls_name,
                                 scores, save_path=save_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save-dir", type=str,
                        default="/content/infernece_images")
    parser.add_argument("--tiling", action="store_true")
    parser.add_argument("--scales", type=str, default="1024",
